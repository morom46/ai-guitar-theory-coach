...
/**
 * A chart written the way you would write it on paper: "Am C D Fmaj7".
 *
 * Stored as an array so it survives a round trip through JSON export, but
 * typed as a string here because a hundred and fifty songs of ["Am", "C"]
 * would be unreadable. Chords are always the SOUNDING chords, the ones the
 * record is actually in. What your hand grabs is that transposed by the capo,
 * which the page works out for you.
 */
const chart = (v) => (Array.isArray(v) ? v : String(v).trim().split(/[\s,|]+/)).filter(Boolean);

// song: id, title, artist, key ("Em"/"D"), scaleId, tone,
//       opts { extras, sections, chords, capo, tuning }
//   capo:   fret the capo sits on in the original, 0 or absent for none
//   tuning: semitones the whole guitar is detuned on the record (-1 = half
//           step down), so the grips can be worked out from the sounding key
const S = (id, title, artist, key, scaleId, tone, opts = {}) => normaliseSong({
  ...
  scaleId,
  extras: opts.extras || [],
  ...(opts.chords ? { chords: chart(opts.chords) } : {}),
  ...(opts.capo ? { capo: opts.capo } : {}),
...
// A section may carry its own chart; without one it inherits the song's.
const sec = (id, name, key, scaleId, pos, chords) => ({
  id, name, root: key.replace(/m$/, ""), scaleId, pos,
  ...(chords ? { chords: chart(chords) } : {}),
});
...
export const SEED_SONGS = [
  ...
  S("s1", "Stairway to Heaven", "Led Zeppelin", "Am", "minorPent",
    ...
    { chords: "Am C/G D/F# Fmaj7 G", extras: [2, 9], sections: [sec("s1a", "Intro", "Am", "aeolian", 0), sec("s1b", "Verse", "Am", "minorPent", 5), sec("s1c", "Solo", "Am", "minorPent", 5), sec("s1d", "Outro", "Am", "minorPent", 12)] }),
  S("s2", "Sweet Child o' Mine", "Guns N' Roses", "D", "majorPent",
    ...
    { chords: "D C G", sections: [sec("s2a", "Intro riff", "D", "majorPent", 12), sec("s2b", "Verse", "D", "majorPent", 2), sec("s2c", "Solo", "E", "minorPent", 12)] }),
  ...
  S("s7", "Knockin' on Heaven's Door", "Guns N' Roses", "G", "majorPent",
    T("Clean Warm", 3, 4, 4, 7, { r: ["Spring", 3] }, "Clean combo + spring reverb", "Open-chord strums. GN'R fills: jump to Super Crunch g6, bridge HB."), { chords: "G D Am C", extras: [3, 10] }),
  ...
  S("s12", "Nothing Else Matters", "Metallica", "Em", "aeolian",
    ...
    { chords: "Em D C G Am B7", sections: [sec("s12a", "Intro", "Em", "aeolian", 0), sec("s12b", "Verse", "Em", "aeolian", 0), sec("s12c", "Solo", "Em", "minorPent", 12)] }),
  S("s13", "Californication", "Red Hot Chili Peppers", "Am", "minorPent",
    T("Clean Warm", 2, 4, 4, 7, { r: ["Room", 2] }, "Marshall Major clean · Strat", "Position 4 hollow pluck; barely brush the strings."), { chords: "Am F C G Dm", extras: [2, 9] }),
  S("s14", "Hotel California (solo)", "Eagles", "Bm", "minorPent",
    ...
    { chords: "Bm F#7 A E G D Em", sections: [sec("s14a", "Verse", "Bm", "aeolian", 0), sec("s14b", "Solo", "Bm", "minorPent", 7)] }),
  S("s15", "Comfortably Numb (solo)", "Pink Floyd", "Bm", "minorPent",
    T("OD 1", 7, 4, 5, 6, { d: ["Tape", 5, 470], r: ["Plate", 3] }, "Hiwatt + Big Muff · Strat neck", "Singing sustain — big slow bends, let repeats breathe."), { chords: "Bm A G Em D C", extras: [2, 9] }),
  S("s16", "Purple Haze", "Jimi Hendrix", "E", "minorPent",
    T("OD 1", 9, 6, 5, 5, {}, "Marshall Super Lead + Fuzz Face", "Fuzz ≈ max gain + neck single + tone ~5; clean up with guitar volume."), { chords: "E7#9 G A", extras: [4] }),
  S("s17", "Back in Black", "AC/DC", "E", "minorPent",
    T("Crunch", 5, 8, 1, 8, {}, "Marshall JMP, no pedals · SG", "Less gain than you think; punch is the right hand. No effects."), { chords: "E D A", extras: [4, 9] }),
  S("s18", "Johnny B. Goode", "Chuck Berry", "Bb", "majorPent",
    T("Clean Bright", 4, 3, 2, 8, { r: ["Spring", 2] }, "Small tweed on the edge · ES-350", "Edge-of-breakup; growls only when you dig in. Double-stops everywhere."), { chords: "Bb Eb F", extras: [3, 10] }),

  /* == classic rock == */
  S("n01", "Highway to Hell", "AC/DC", "A", "majorPent",
    T("Crunch", 5, 8, 1, 8, {}, "Marshall plexi crunch · SG", "Open A-D-G cowboy-chord riff; stop between chords, no wash."), { chords: "A D/F# G D", extras: [3, 10] }),
  ...
  S("n03", "You Shook Me All Night Long", "AC/DC", "G", "majorPent",
    T("Crunch", 5, 8, 1, 8, {}, "Marshall · SG", "G-C-D with double-stop fills from G major pentatonic."), { chords: "G C D", extras: [3, 10] }),
  ...
  S("n09", "La Grange", "ZZ Top", "Am", "minorPent",
    T("Crunch", 5, 6, 1, 8, {}, "Marshall-ish · Les Paul, pinch harmonics", "A-blues shuffle; the squeals are pinch harmonics on the pentatonic."), { chords: "A C D", extras: [4, 6] }),
  S("n10", "Sharp Dressed Man", "ZZ Top", "C", "minorPent",
    T("Crunch", 6, 6, 1, 8, {}, "Tight Marshall crunch", "C blues riff; slide into everything."), { chords: "C F G", extras: [6] }),
  S("n11", "Sweet Home Alabama", "Lynyrd Skynyrd", "D", "majorPent",
    T("Clean Bright", 4, 3, 2, 8, { r: ["Spring", 2] }, "Fender clean edge · Strat/LP mix", "D-C-G loop; fills from D major pentatonic. Keep it twangy — position 2."), { chords: "D C G", extras: [3, 10] }),
  S("n12", "Free Bird", "Lynyrd Skynyrd", "G", "majorPent",
    ...
    { chords: "G D/F# Em F C D", sections: [sec("n12a", "Verse", "G", "major", 0), sec("n12b", "Solo", "Em", "minorPent", 12)] }),
  S("n13", "Simple Man", "Lynyrd Skynyrd", "Am", "minorPent",
    T("Clean Warm", 3, 4, 5, 7, { r: ["Room", 3] }, "Clean arpeggios, then pushed leads", "C-G-Am arpeggio loop; solo from Am pentatonic with OD 1 g6."), { chords: "C G Am", extras: [2, 9] }),
  ...
  S("n19", "Crazy Train", "Ozzy Osbourne", "F#m", "minorPent",
    T("Super Crunch", 7, 6, 1, 9, { r: ["Room", 2] }, "Marshall + MXR Distortion+ · Randy's LP", "Riff is F# minor with a natural 6 (Dorian flavour) — add the 9/6 extras."), { chords: "F#m A E", extras: [2, 9] }),
  S("n20", "Sweet Emotion", "Aerosmith", "A", "minorPent",
    T("Crunch", 5, 6, 1, 7, {}, "Marshall · LP, talkbox intro", "Bass owns the intro; guitar answers with A pent stabs and slides."), { chords: "A E D", extras: [4] }),
  ...
  S("n87", "Choo Lo", "The Local Train", "B", "major",
    ...
    { chords: "B E A C#m", sections: [
      // The loop that runs under everything — B-E-A-C#m, read off the bass.
      sec("n87a", "The B-E-A-C#m loop", "B", "major", 4, "B E A C#m"),
      sec("n87b", "Outro solo — the low half", "B", "major", 9, "B E A C#m"),
      // Bars 124-128 sit at frets 17-21.
      sec("n87c", "Outro solo — the top", "B", "major", 16, "B E A C#m"),
    ] }),

  /* == blues / blues-rock == */
  S("n25", "Pride and Joy", "Stevie Ray Vaughan", "E", "minorPent",
    T("Clean Bright", 5, 3, 4, 8, { r: ["Spring", 2] }, "Super Reverb cranked · Strat, heavy strings", "Texas shuffle — thumb hits low E on every beat. Dig in HARD; the amp edge is your drive."), { chords: "E A B", extras: [4, 6] }),
  S("n26", "The Thrill Is Gone", "B.B. King", "Bm", "minorPent",
    T("Clean Warm", 4, 4, 3, 7, { r: ["Room", 3] }, "Lab Series clean · Lucille (ES-355)", "One-note-at-a-time phrasing, butterfly vibrato. Say something, then stop."), { chords: "Bm Em Gmaj7 F#7", extras: [2, 9] }),
  ...
  S("n29", "Cocaine", "Eric Clapton", "E", "minorPent",
    T("Crunch", 5, 6, 2, 7, {}, "Tweed pushed · Strat", "E-D riff with lazy behind-the-beat pentatonic fills."), { chords: "E D A", extras: [4, 6] }),
  ...
  S("n31", "Tears in Heaven", "Eric Clapton", "A", "major",
    ...
    { chords: "A E/G# F#m D E" }),
  S("n32", "Folsom Prison Blues", "Johnny Cash", "E", "majorPent",
    T("Clean Bright", 4, 3, 2, 8, { r: ["Spring", 2] }, "Small tweed · Tele twang", "Boom-chicka rhythm — mute the bass strings with your palm the whole time."), { chords: "E A B", extras: [3, 10] }),
  ...
  S("n34", "Alive", "Pearl Jam", "A", "majorPent",
    T("Crunch", 6, 6, 1, 8, { r: ["Room", 2] }, "Marshall · LP", "Riff is A mixolydian-ish; solo switches to A minor pent — classic trick."), { chords: "A E/G# G D", extras: [3, 10] }),
  ...
  S("n43", "Wonderwall", "Oasis", "F#m", "minorPent",
    T("Crunch", 3, 6, 3, 7, { r: ["Room", 3] }, "Acoustic on record (capo 2)", "Capo 2, Em shapes. Keep the top two strings ringing in every chord."), { chords: "F#m7 A Esus4 B7sus4 Dadd9", capo: 2, extras: [2, 9] }),
  ...
  S("n47", "Basket Case", "Green Day", "Eb", "major",
    ...
    { chords: "Eb Bb Cm Gm Ab", tuning: -1 }),
  ...
  S("n55", "Fade to Black", "Metallica", "Bm", "minorPent",
    ...
    { chords: "Bm G D A", sections: [sec("n55a", "Intro", "Bm", "aeolian", 0), sec("n55b", "Verse", "Bm", "aeolian", 0), sec("n55c", "Outro", "Em", "minorPent", 12)] }),
  ...
  S("n57", "Little Wing", "Jimi Hendrix", "Em", "minorPent",
    T("Clean Warm", 4, 5, 4, 7, { r: ["Room", 3] }, "Fender clean + Strat pos 4, thumb chords", "Chord-melody: thumb over the neck, hammer-on embellishments inside chords."), { chords: "Em G Am Bm Bb C D F", extras: [2, 9] }),
  S("n58", "Hey Joe", "Jimi Hendrix", "E", "minorPent",
    T("Crunch", 4, 5, 4, 7, {}, "Just-breaking-up Marshall", "C-G-D-A-E cycle; fills from E pentatonic between every chord."), { chords: "C G D A E", extras: [3, 10] }),
  ...
  S("n63", "Day Tripper", "The Beatles", "E", "majorPent",
    T("Crunch", 4, 5, 2, 8, {}, "Vox top-boost edge", "The riff IS the song — E major pent with the b3 passing note."), { chords: "E A B", extras: [3] }),
  ...
  S("n69", "Brown Eyed Girl", "Van Morrison", "G", "major",
    T("Clean Bright", 3, 3, 2, 8, { r: ["Spring", 2] }, "Bright clean combo", "The intro double-stop thirds run G major — slow it down, it's a great scale drill."), { chords: "G C D Em", extras: [3, 10] }),
  ...
  S("n75", "Should I Stay or Should I Go", "The Clash", "D", "majorPent",
    T("Crunch", 5, 6, 1, 8, {}, "Raw Tele into cooking amp", "D-G-D stabs with stops — mute HARD in the gaps."), { chords: "D G A F", extras: [3, 10] }),
  ...
  S("n81", "Dani California", "Red Hot Chili Peppers", "Am", "minorPent",
    T("Crunch", 5, 5, 4, 7, {}, "Hendrix-y verse, big chorus", "Am-G-Dm loop; outro solo = Am pent, OD 1 g7, wah faked with pick position."), { chords: "Am G Dm", extras: [2, 9] }),
  ...
  S("n82", "Wish You Were Here", "Pink Floyd", "G", "major",
    ...
    { chords: "G Em A7sus4 C D Am" }),
  S("n83", "Time (solo)", "Pink Floyd", "F#m", "minorPent",
    T("OD 1", 7, 4, 5, 6, { d: ["Tape", 4, 480], r: ["Plate", 3] }, "Hiwatt + Muff · Strat neck", "Bends up to the 9th — F#m pent + the 9 (add extras). Slow hands, big vibrato."), { chords: "F#m A E", extras: [2, 9] }),
  S("n84", "Another Brick in the Wall Pt. 2 (solo)", "Pink Floyd", "Dm", "minorPent",
    T("Crunch", 5, 4, 2, 8, { d: ["Analogue", 3, 440] }, "Funky verse · LP neck solo", "Verse: tight muted clean funk. Solo: Dm pent, every phrase lands on a chord tone."), { chords: "Dm Gm C", extras: [2, 9] }),
  ...
  S("n88", "In Any Tongue", "David Gilmour", "Cm", "aeolian",
    ...
    { chords: "Cm Fm G7", sections: [
      sec("n88a", "Verse / piano intro (Cm-Fm-G7)", "Cm", "aeolian", 8, "Cm Fm G7"),
      // The solo's own key. Fm is Cm's iv, and F aeolian is Ab major.
      sec("n88b", "Solo — the low half (frets 1-13)", "Fm", "aeolian", 3, "Fm Ab Db Eb"),
      // Bars 1-25 live at frets 15-18 on the B and high E strings.
      sec("n88c", "Solo — the top (frets 13-18)", "Fm", "aeolian", 15, "Fm Ab Db Eb"),
    ] }),
];
...
/**
 * CHARTS ARRIVED AFTER THE LIBRARY DID.
 *
 * A saved song beats its seed, which is right for everything you edited and
 * wrong for a field that did not exist when you saved it. Every one of the
 * seeded songs is already sitting in localStorage on any browser that has
 * opened this app, so `chords`, `capo` and `tuning` would never be seen.
 *
 * So fill in only what the saved copy has no opinion about. A key that is
 * PRESENT is your answer, an empty chart included, which is how clearing a
 * chart survives a reload instead of growing back.
 */
const CHART_KEYS = ["chords", "capo", "tuning"];

export function backfillChart(saved, seed) {
  if (!saved || !seed) return saved;
  let out = saved;
  for (const k of CHART_KEYS) {
    if (!(k in out) && k in seed) out = { ...out, [k]: seed[k] };
  }
  if (seed.sections?.length && out.sections?.length) {
    const bySeed = new Map(seed.sections.map((x) => [x.id, x]));
    let touched = false;
    const sections = out.sections.map((x) => {
      const hit = bySeed.get(x.id);
      if (!hit || !hit.chords || "chords" in x) return x;
      touched = true;
      return { ...x, chords: hit.chords };
    });
    if (touched) out = { ...out, sections };
  }
  return out;
}
...
export const loadSongs = () => {
  ...
  if (v2 && v2.length) {
    ...
    const seedById = new Map(SEED_SONGS.map((s) => [s.id, s]));
    // Saved songs get the same blues promotion as the seeds — including ones
    // the user wrote themselves — and the chart fields the seeds grew later.
    return [
      ...v2.map((s) => backfillChart(normaliseSong(s), seedById.get(s.id))),
      ...SEED_SONGS.filter((s) => !ids.has(s.id)),
    ];
  }
...
SEED_SONGS.push(
  ...
  S("m07", "Take It Easy", "Eagles", "G", "majorPent",
    ...
    { chords: "G D C Am Em", extras: [3, 10] }),
  ...
  S("m32", "Billie Jean", "Michael Jackson", "F#m", "minorPent",
    ...
    { chords: "F#m G#m Bm E", extras: [2, 9] }),
  ...
  S("m39", "Crazy Little Thing Called Love", "Queen", "D", "majorPent",
    ...
    { chords: "D G C Bb Eb", extras: [3, 10] }),
